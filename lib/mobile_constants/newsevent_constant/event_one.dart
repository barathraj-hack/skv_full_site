...
class MyEventOneMobile extends StatefulWidget {
  const MyEventOneMobile({super.key});

  @override
  State<MyEventOneMobile> createState() => _MyEventOneMobileState();
}

class _MyEventOneMobileState extends State<MyEventOneMobile> {
  // List of image paths
  ...
  Future<void> fetchFirebaseImagesThree() async {
    final storageRef = FirebaseStorage.instance
        .ref()
        .child('uploaded_images_seven/'); // Folder 3
  ...
  Future<void> fetchFirebaseImagesFour() async {
    final storageRef = FirebaseStorage.instance
        .ref()
        .child('uploaded_images_eight/'); // Folder 4
    final ListResult result = await storageRef.listAll();
    final urls =
        await Future.wait(result.items.map((ref) => ref.getDownloadURL()));
  ...
  Future<void> fetchNewsText() async {
    try {
      final snapshot = await FirebaseFirestore.instance
          .collection('uploaded_texts_four')
          .orderBy('timestamp',
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print("⚠️ No documents found in 'uploaded_texts_four'");
      }
    } catch (e) {
  ...
  Future<void> fetchNewsOneText() async {
    try {
      final snapshot = await FirebaseFirestore.instance
          .collection('uploaded_texts_five')
          .orderBy('timestamp',
              descending: true) // make sure your docs have timestamp
          .limit(1) // get only the latest
          .get();
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print("⚠️ No documents found in 'uploaded_texts_five'");
      }
    } catch (e) {
      print("❌ Error fetching news text: $e");
    }
